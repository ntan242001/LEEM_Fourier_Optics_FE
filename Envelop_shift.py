...
epsilon_0_series = np.linspace(-0.5, -0.5, 11)

# Initialising the series of function I_1(x) and I_2(x) at different values of q_ap
matrixE_ct = np.zeros((len(x_array), len(epsilon_0_series)), dtype=complex)
...
def Envelop1Gauss(epsilon_0, epsilon_0_index):
...
for mat in parallelResult:
    matrixE_ct = np.hstack(matrixE_ct, mat)
...
for i in range(len(epsilon_0_series)):
    ...
    plt.title('$$E_{ct,tot}(q,0)$ versus q, epsilon_0 = ' + str(epsilon_0_series[i]) )
    plt.legend()
