...
alpha_ill_series = np.linspace(1*1e-3, 5*1e-3, 17)

q_ill_series = alpha_ill_series/lamda

# Initialising the series of function I_1(x) and I_2(x) at different values of q_ill
matrixI1 = np.zeros((len(x_array), len(q_ill_series)), dtype=complex)
matrixI2 = np.zeros((len(x_array), len(q_ill_series)), dtype=complex)

# A function to calculate the image for single Gaussian distribution
def Image1(q_ill, q_ill_index):    
    matrixI = np.zeros((len(x_array), len(q_ill_series)), dtype=complex)
    ...
    for i in range(len(q)):
        for j in range(i + 1, len(q)):
            matrixI[:, q_ill_index] = matrixI[:, q_ill_index] + 2 * (
                    AR[j][i] * np.exp(1j * 2 * np.pi * (Q[j][i] - QQ[j][i]) * x_array)).real
        

    matrixI[:, q_ill_index] = matrixI[:, q_ill_index] + np.trace(AR) * np.ones_like(x_array)

    return matrixI

with Parallel(n_jobs=-1, verbose=50, max_nbytes="50M") as parallel:
    parallelResult = parallel(delayed(Image1)(q_ill, q_ill_index) for q_ill_index, q_ill in enumerate(q_ill_series))
...
with Parallel(n_jobs=-1, verbose=50, max_nbytes="50M") as parallel:
    parallelResult = parallel(delayed(Image2)(q_ill, q_ill_index) for q_ill_index, q_ill in enumerate(q_ill_series))
...
# plotting the curves
for i in range(len(alpha_ill_series)):
    plt.plot(x_array, matrixI1[:, i], label = '1st order')
    plt.plot(x_array, matrixI2[:, i], label = '2nd order')
...
'''      
# Finding lists of resolutions corresponding to different aperture angles 
resolution_list1 = []
resolution_list2 = []
def Find_Resolution(resolution_list, matrixI):
    for i in range(len(alpha_ap_series)):
        ...
        if object_type == "Step phase object" or object_type == "Error function phase object":
            ...
            idx_left = np.argmin(np.abs(matrixI_i[idx_left:idx_min] - 1))
            I_left = matrixI_i[idx_left]
            idx_right = np.argmin(np.abs(matrixI_i[idx_min:idx_right] - 1))
            I_right = matrixI_i[idx_right]
...
Find_Resolution(resolution_list1, matrixI1)
Find_Resolution(resolution_list2, matrixI2)
    
plt.plot(alpha_ap_series*1e3, resolution_list1)
plt.plot(alpha_ap_series*1e3, resolution_list2)
# naming the x axis
...
R_ac = []
R_nac = []
with open('Step phase object_R(a)_IBM_ac.csv', 'r') as csvfile:
    ...
    csvfile.close()
with open('Step phase object_R(a)_IBMnac.csv', 'r') as csvfile:
    csv_reader = csv.reader(csvfile, delimiter=',')
    line_count = 0
    for row in csv_reader:
        if line_count == 0:
