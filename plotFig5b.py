...
xmax = 10
ytop = 1.2
xticks = np.array([-10. ,  -7.5,  -5. ,  -2.5,   0. ,   2.5,   5. ,   7.5,  10. ])
yticks = [0.4,0.5,0.6,0.7,0.8,0.9,1.0,1.1]
...
i=0
ax.plot(x_array*1e9, matrixIFN[:, i], 'r-',linewidth=2, label='FE')
ax.plot(x_array*1e9, matrixI1[:, i], 'k--',linewidth=2, label='Gaussian')
...
ax.set_yticklabels(yticks,fontsize=s-2)

ax.set_ylabel('Intensity', fontsize=s)
ax.set_xlabel('Position (nm)', fontsize=s)

ax.text(-13.2, 1.09, '(b)',fontsize=s)

def find1684lines(FE=True):
    if FE==True:
        matrixI_i = matrixIFN
    else:
        matrixI_i = matrixI1
...
ax.axvline(find1684lines(True)[0]*1e9, color='r', linestyle='-',linewidth=0.9)
ax.axvline(find1684lines(True)[1]*1e9, color='r', linestyle='-',linewidth=0.9)
ax.axvline(find1684lines(False)[0]*1e9, color='k', linestyle='-',linewidth=0.9)
ax.axvline(find1684lines(False)[1]*1e9, color='k', linestyle='-',linewidth=0.9)

ax.legend(frameon=False, fontsize=s)
plt.savefig("Figure_5b.png", dpi=1000)
