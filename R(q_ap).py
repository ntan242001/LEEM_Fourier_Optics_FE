...
matrixIFN = np.zeros((len(x_array), len(q_ap_series)), dtype=complex)


def FO1G(q_ap, q_ap_index):    
...
#     parallelResult = parallel(delayed(FO1G)(q_ap, q_ap_index) for q_ap_index, q_ap in enumerate(q_ap_series))
...
with Parallel(n_jobs=-1, verbose=50, max_nbytes="50M") as parallel:
    parallelResult = parallel(delayed(FOFN)(q_ap, q_ap_index) for q_ap_index, q_ap in enumerate(q_ap_series))
...
#     for i in range(len(alpha_ap_series)):
#         writer.writerow([alpha_ap_series[i], 1e9 * resolution_list1[i]])
...
with open('Step amplitude object_R(a)_FN_ac.csv', 'a') as csvfile:
    ...
    for i in range(len(alpha_ap_series)):
        writer.writerow([alpha_ap_series[i], 1e9 * resolution_listFN[i]])
