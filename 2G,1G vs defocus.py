...
delta_z_series = np.append(np.linspace(-9*1e-6, -4*1e-6, 3), np.linspace(-3.5*1e-6, 3.5*1e-6, 9))
delta_z_series = np.append(delta_z_series, np.linspace(4*1e-6, 10*1e-6, 3))
'''
delta_z_series = np.linspace(-5*1e-6, 5*1e-6, 3)
'''
...
# A function to calculate the image for double Gaussian distribution 
def Image2GaussC1(delta_z, delta_z_index):
    matrixI = np.zeros((len(x_array), len(delta_z_series)), dtype=complex)
    # The Fourier Transform of the Object Wave Function
    F_object_function = np.fft.fft(object_function_reversed, simulating_steps) * (1 / simulating_steps)
    # Shifting this to the centre at 0
...
with Parallel(n_jobs=-1, verbose=50, max_nbytes="50M") as parallel:
    parallelResult = parallel(delayed(Image2GaussC1)(delta_z, delta_z_index) for delta_z_index, delta_z in enumerate(delta_z_series))
...
for i in range(len(delta_z_series)):
    ...
    plt.plot(x_array, matrixI3[:, i], label = '1 of Triple Gauss')
...
plt.plot(delta_z_series*1e6, resolution_list2, label = '1 of Triple Gaussian')
