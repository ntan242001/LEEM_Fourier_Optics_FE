...
with open('R(dz) G1 nac_LEEM.csv', 'r') as csvfile:
...
with open('R(dz) FN nac_LEEM.csv', 'r') as csvfile:
    csv_reader = csv.reader(csvfile, delimiter=',')
    line_count = 0
    for row in csv_reader:
        if line_count == 0:
...
smooth_R_G1_nac = savgol_filter(R_G1_nac, 33, 9)
smooth_R_FN_nac = savgol_filter(R_FN_nac, 33, 9)
...
ax.plot(delta_z_series_nac/((C_3*lamda)**(1/2)), smooth_R_G1_nac, 'm-', label = "NAC, Gaussian")
ax.plot(delta_z_series_nac/((C_3*lamda)**(1/2)), smooth_R_FN_nac, 'c-', label = "NAC, FN")
ax.plot(delta_z_series_ac/((C_5*lamda**2)**(1/3)), smooth_R_G1_ac, 'r-', label = "AC, Gaussian")
ax.plot(delta_z_series_ac/((C_5*lamda**2)**(1/3)), smooth_R_FN_ac, 'b-', label = "AC, FN")
...
ax.axvline(x=6.27e-7/((C_3*lamda)**(1/2)), linestyle = '--', c = 'm', linewidth=5)
ax.axvline(x=1.36e-6/((C_3*lamda)**(1/2)), linestyle = '--', c = 'c')
